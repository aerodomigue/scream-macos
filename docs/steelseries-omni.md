...
A separate **headphones + percentage** item appears in the menu bar only in this mode. The headset icon turns orange below 20% and red below 10%; at 20% or above, or when the battery level is unavailable, it stays white. The percentage keeps its normal color, white on the active display and light gray on an inactive display. Move it independently with **Command-drag**. Clicking it opens the battery popup; clicking outside closes it. The main application icon remains available for settings and PC status.
...
**Volume + / − and Mute keys** control the base's hardware volume while Omni mode is active, the headset is connected, and Omni is the Mac's default sound output. Click **Allow volume keys…** in Status or the headset popup and enable ScreamBar in **System Settings → Privacy & Security → Accessibility**. Each press moves one hardware step (about 1.8 percentage points); holding a key repeats. Knob notches and key presses are sent individually, without an artificial delay between volume writes. The displayed percentage follows key presses immediately and is checked against the base after the burst. Accepted keys also show a compact macOS-style volume indicator near the upper-right corner of the screen containing the pointer; it disappears 1.5 seconds after the last press without taking focus or adding USB polling.

### Volume feedback sound

ScreamBar follows **System Settings → Sound → Play feedback when volume is changed**, including changes made while ScreamBar is running. After a successful volume write, it restarts the native macOS volume sound on the Omni output, without waiting for the previous sound to finish. Muting, unmuting, unchanged volume limits, zero volume, background polling and final verification do not trigger a sound. Mute, disconnection and output changes stop existing feedback. The setting being off always means silence, including with Shift held; there is no separate ScreamBar sound toggle. Feedback errors are logged without preventing hardware volume control.

The preference key and native sound file location are macOS implementation details. If either is unavailable or the preference cannot be read, ScreamBar stays silent. The sound previews the headset's hardware volume; the separate macOS alert volume and sound-effects output selection are not applied.

### USB traffic and responsiveness

ScreamBar reads the current volume at the start of each key sequence, then sends changes using the last successfully written value. It verifies the result after 150 ms without a new press; this quiet interval delays only verification, never writes. Settings reads retain a 50 ms response-settling interval, while volume writes have no added pause. Dial changes are picked up at the start of the next sequence. Holding a key at 0% or 100% sends no redundant volume commands. Both views share one volume refresh about once a second while visible; closing them stops volume polling. Battery status is still checked every five seconds in Omni mode. Unchanged values do not trigger interface updates. USB operations run serially in the background; pending adjustments are cancelled when the mode or output changes. If a blocked device fills the 128-action input queue, further inputs are ignored and an explicit status message is shown. The macOS Control Center volume slider is not handled by this feature. These controls do not change the Mac's selected audio output or process its existing audio streams.
